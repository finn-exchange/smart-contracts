...
// The FINN Trading Contract
contract FINN_Trading {
    ...
    // Allows only the owner of the contract to execute the function
    modifier onlyOracle {
        assert(msg.sender == FinnOracleContract);
        _;
    }
    ...
    address public exchangeContract;    // the address of the FINN_Base contract
    address public FinnOracleContract;  // the address of the FINN oracle contract
    ...
    function FINN_Trading(address feeAccount_, uint256 makerFee_, uint256 takerFee_, address exchangeContract_, address FinnOracleContract_, address poolAddress) {
        owner               = msg.sender;
        ...
        FinnOracleContract    = FinnOracleContract_;
    ...
    ) onlyAdmin returns (uint filledTakerTokenAmount)
    {
        ...
        */
        ...
        uint256[4] memory balances = FINN_Base(exchangeContract).getMakerTakerBalances(t.baseToken, t.maker, t.taker);
...
//--> 264 000
    ...
    function updateBalances (bytes32 futuresContract, address[3] addressValues, bytes32 positionHash, uint256[9] uintValues, bool[3] boolValues) private returns (bool)
    {
        ...
        */
        ...
        if (pam[0] > 0)
        {
            addBalance(addressValues[0], feeAccount, FINN_Base(exchangeContract).balanceOf(addressValues[0], feeAccount), pam[0]); // send fee to feeAccount
        }

        if (pam[2] > 0)
        {
            addBalance(addressValues[0], addressValues[2], FINN_Base(exchangeContract).balanceOf(addressValues[0], addressValues[2]), safeMul(pam[2], 1e10));
        }
    ...
    {   
        ...
        if (safeAdd(futuresContracts[futuresContract].expirationBlock, FINN_Base(exchangeContract).getInactivityReleasePeriod()) > block.number) revert();    
    ...
    function forceReleaseReserveOperation(bytes32 futuresContract, bool side, address user) private
    {
        ...
        subReserve(
            baseToken, 
            user, 
            FINN_Base(exchangeContract).getReserve(baseToken, user), 
            pos[4]
        );        
    ...
    {
        FINN_Base(exchangeContract).setBalance(token, user, safeAdd(balance, amount));
    }

    function subBalance(address token, address user, uint256 balance, uint256 amount) private returns (bool)
    {
        if (balance < amount) return false;
        FINN_Base(exchangeContract).setBalance(token, user, safeSub(balance, amount));
        return true;
    }

    function subBalanceAddReserve(address token, address user, uint256 subBalance, uint256 addReserve) private returns (bool)
    {
        if (!FINN_Base(exchangeContract).subBalanceAddReserve(token, user, subBalance, safeMul(addReserve, 1e10)))
        {
            return false;
        }
    ...
    {
        if (!FINN_Base(exchangeContract).addBalanceSubReserve(token, user, addBalance, safeMul(subReserve, 1e10)))
        {
            return false;
        }
    ...
    {
        if (!FINN_Base(exchangeContract).subBalanceSubReserve(token, user, subBalance, safeMul(subReserve, 1e10)))
        {
            return false;
        }
    ...
    {
        FINN_Base(exchangeContract).setReserve(token, user, safeSub(reserve, safeMul(amount, 1e10)));
    }
    ...
    function closeFuturesPositionInternal (bytes32 futuresContract, bool side, address user, address poolAddress, uint256 expirationFee) private returns (bool)
    {
        ...
        v.reserve = FINN_Base(exchangeContract).getReserve(v.baseToken, user);
        v.balance = FINN_Base(exchangeContract).balanceOf(v.baseToken, user);
        ...
        {   
            ...
            {
                if (!subBalance(v.baseToken, poolAddress, FINN_Base(exchangeContract).balanceOf(v.baseToken, poolAddress), safeMul(safeSub(v.profit, safeAdd(v.fundingCost, v.closingFee/2)), 1e10)))
                {
                    // brake contract
                    futuresContracts[futuresContract].broken = true;
                    forceReleaseReserveOperation(futuresContract, side, user);
            ...
            {
                addBalance(v.baseToken, poolAddress, FINN_Base(exchangeContract).balanceOf(v.baseToken, poolAddress), safeMul(safeSub(safeAdd(v.fundingCost, v.closingFee/2), v.profit), 1e10)); 
            }
        ...
        else
        {
            ...
            addBalance(v.baseToken, poolAddress, FINN_Base(exchangeContract).balanceOf(v.baseToken, poolAddress), safeMul(safeSub(min(v.collateral, safeAdd(v.loss, v.totalPayable)), v.closingFee/2), 1e10)); 
        } 
        ...
        addBalance(v.baseToken, feeAccount, FINN_Base(exchangeContract).balanceOf(v.baseToken, feeAccount), safeMul(v.closingFee/2, 1e10)); // send fee to feeAccount
    ...
    function updateMargin(bytes32 futuresContractHash, address user, bool side, uint8 vs, bytes32 r, bytes32 s, uint64 newMargin /* 1e8 */, uint256 operationFee /* 1e18 */)
    {
        ...
        if (newMargin > pos[4])
        {
            if (FINN_Base(exchangeContract).availableBalanceOf(v.baseToken, user) < safeMul(safeAdd(safeSub(newMargin, pos[4]), fee), 1e10)) revert();
        }        
        ...
        addBalance(v.baseToken, feeAccount, FINN_Base(exchangeContract).balanceOf(v.baseToken, feeAccount), safeMul(fee, 1e10));
    ...
    function liquidatePosition (bytes32 positionHash, bytes32 futuresContractHash, address user, bool side, uint256 price, address poolAddress, uint256 block) private
    {
        ...
        addBalance(futuresAssets[assetHash].baseToken, poolAddress, FINN_Base(exchangeContract).balanceOf(futuresAssets[assetHash].baseToken, poolAddress), safeMul(collateral, 1e10));
